...
for SUB in ${SubjectID} ; do
	...
	echo "singularity exec ${ContainerPath}/fmriprep-1.1.8.simg fmriprep ${DataPath} ${fmriprepOut} participant -w ${fmriprepWork} -t ${TaskID} --participant-label ${SUB} --fs-license-file ${FSlicense} --output-space T1w template fsnative fsaverage -n-cpus ${cpus} --mem-mb ${mem_mb} --verbose --write-graph --notrack" >> job
